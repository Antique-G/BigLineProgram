...
import { AdminUserinfoListResponseModel, DetailModel, SetStatusRequestModel, UpdateInfoModel, UserOrderModel } from '../../interfaces/adminUserinfo/admin-userinfo-model';
...
export class AdminUserinfoService {
    ...
    // 获取用户对应的订单
    userOrder(page: number, per_page: number, user_id: any): Observable<UserOrderModel> {
        const params = new HttpParams().set('page', page.toString())
            .set('per_page', per_page.toString())
        ...
        return this.httpClient.get<UserOrderModel>(this.urls.GetAdminUserOrderIndex, findhttpOptions)
            .pipe(
                catchError(this.handleError)
            )
    }
