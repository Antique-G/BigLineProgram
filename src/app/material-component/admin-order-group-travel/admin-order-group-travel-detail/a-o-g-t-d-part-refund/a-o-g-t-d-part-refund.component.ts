...
export class AOGTDPartRefundComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.price_diff = '￥' + this.detailModel.price_diff + '*' + this.detailModel?.num_diff;
        this.difRoom = this.detailModel?.num_diff;
        ...
        //最开始剩余房间数为原始的
        if (this.isType == 1) {
            this.refundRoomNum = this.detailModel.num_room;
        }
        else {
        ...
        // 退款方案  this.isType = this.isType === 0 ? "全部退款" : "部分退款"; 全部退款就全选
        if (this.isType === 0) {
            this.selectMemberData.forEach((ele: any) => {
                ele['disabled'] = true;
    ...
    priceAll() {
        ...
        //剩余房差房间数
        this.difRoom = Number(this.refundRoomNum) * 2 - Number(remain) - Number(this.detailModel?.shared_status);
        ...
        console.log('this.difRoom1111', Number(this.refundRoomNum) * 2, Number(this.detailModel?.shared_status), this.difRoom);
