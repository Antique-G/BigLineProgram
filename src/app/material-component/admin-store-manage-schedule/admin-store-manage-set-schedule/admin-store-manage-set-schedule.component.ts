...
export class AdminStoreManageSetScheduleComponent implements OnInit {
  ...
  setValue() {
    ...
    this.resultArr.shop_id = this.addForm.value.shop_id;
  ...
  changeShop(data: any) {
    console.log(data);
    let i = this.shopList.filter(item => item.id == data);
    console.log('i :>> ', i);
    this.region_code = i[0]?.region_code;
    this.addForm.patchValue({
      admin_id: []
    })
    this.adminStoreManageService.shopAccountList(this.region_code).subscribe(res => {
      console.log('选择完后的 ', res);
      this.adminList = res.data;
    })
